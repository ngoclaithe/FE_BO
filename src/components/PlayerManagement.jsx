...
import { getPendingTradesByPhonezalo, updateTrade } from '../services/apiTrade';
import { updateMarketFromAdmin } from '../services/apiMarket';

const PlayerManagement = ({ token }) => {
    const [phonezalo, setPhonezalo] = useState('');
    const [pendingTradesByPhonezalo, setPendingTradesByPhonezalo] = useState([]);
    const [selectedTrade, setSelectedTrade] = useState(null);
    const [currentPrice, setCurrentPrice] = useState(0);
    const [intervalId, setIntervalId] = useState(null);

    const fetchPendingTradesByPhonezalo = async () => {
        if (!phonezalo || phonezalo.length !== 10) {
            return;
        }
        try {
            const data = await getPendingTradesByPhonezalo(phonezalo, token);
            setPendingTradesByPhonezalo(data.pending_trades);
        } catch (error) {
            console.error('Error fetching pending trades by phonezalo:', error);
        }
    };
    ...
    const startInterval = () => {
        ...
        const id = setInterval(fetchPendingTradesByPhonezalo, 10000);
        setIntervalId(id);
    };
    ...
    useEffect(() => {
        if (phonezalo && phonezalo.length === 10) {
            fetchPendingTradesByPhonezalo();
            startInterval();
        } else {
            stopInterval();
            setPendingTradesByPhonezalo([]);
        }
    ...
    return (
        <div className="bg-white p-8 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold text-gray-800">Quản lý người chơi</h2>
            <p className="text-gray-600 mt-2">Nhập số Zalo để xem các giao dịch đang đặt.</p>
            ...
            <div className="mt-6 overflow-x-auto">
                {pendingTradesByPhonezalo.length === 0 ? (
                    ...
                    <table className="min-w-full bg-white border border-gray-200">
                        ...
                        <tbody>
                            {pendingTradesByPhonezalo.map((trade) => (
